...
public class PracticesFragment extends BaseFragment {
   ...
   private PracticeSelectedListener listener;
    ...
    private static  class DownloadHandler extends AbstractStaticHandler<PracticesFragment>{
        ...
        DownloadHandler(PracticesFragment countext) {
            super(countext);
        }
        static class QuestionDownloader extends AsyncTask<Void,Void,String>{
            ...
            protected String doInBackground(Void... voids) {
                try {
                    return QuestionService.getQuestionOfPracticeFromService(practice.getApiId());
                } catch (IOException e) {
                    return  e.getMessage();
                }
            }
        ...
        public void handleMessage(Message msg, PracticesFragment fragment) {
           switch (msg.what){
               ...
               case WHAT_QUESTION_DONE:
                   ...
                   fragment.saveQuestion(msg.obj.toString(), practiceId);
                   ViewUtils.dismissProgress();
                   break;
    ...
    private void saveQuestion(String json, UUID practiceId) {
        ...
        try{
            ...
            factory.saveQestions(questions,practiceId);
            for (Practice practice:practices){
    ...
    private  void loadPractices(){
        ...
        adapter=new GenericAdapter<Practice>(getContext(),R.layout.practices_time,practices) {
            @Override
    ...
    private void downloadQuestion(int apiId) {
        ...
        executor.execute(()-> {
                try {
                    String json= QuestionService.getQuestionOfPracticeFromService(apiId);
    ...
    private  void initViews() {
        ...
        swipe=find(R.id.fragment_practice_swipe);
       tvHint=find(R.id.fragment_practice_tv_hint);
        tvTime=find(R.id.fragment_practice_tv_time);
        tvTime.setText(UserCookies.getInstance().getLastRefreshTime());
        tvHint.setVisibility(View.GONE);
        tvTime.setVisibility(View.GONE);
        find(R.id.fragment_practice_tv_lv).setOnTouchListener(new ViewUtils.AbstractTouchHandler() {
            @Override
            public boolean handleTouch(MotionEvent event) {
                isDelete=false;
                adapter.notifyDataSetChanged();
